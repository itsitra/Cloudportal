...
@Component({
  ...
  templateUrl: 'loginjtc.component.html',
  styleUrls: ['loginjtc.component.css']
})
export class LoginComponentJTC {
  loginFormData = {
    email : ''
    , password : ''
  };
      ...
      this.http.post<any>(environment.apiUrl + 'validate_wallet_login', { data: this.loginFormData }).subscribe({
          next: data => {
            ...
            if (return_values.status === true) {
              ...
              localStorage.setItem('sessiontype', 'jtc');
  ...
  goToRegisteration(): void {
    ...
    // sessionStorage.setItem('sessionregistertype','jtc')
  }
  downloadPdf(): void {
